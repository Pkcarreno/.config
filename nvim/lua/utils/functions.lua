...
M.first_vim_run = function()
  local is_first_run = utils.file_exists('/tmp/first-vim-run')

  if is_first_run then
    async.run(function()
      require('notify')("Welcome! Hope you'll have a nice experience!", "info", { title = "PKvim", timeout = 5000 })
      require('notify')("Please install treesitter servers manually by :TSInstall command.", "info", { title = "Installation", timeout = 10000 })
    end)
    local suc = os.remove('/tmp/first-vim-run')
    if (not suc) then print("Error: Couldn't remove /tmp/first-vim-run!") end
  end
end
